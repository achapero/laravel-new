...
import PageGuideSubAddEditModal from "./Modals/PageGuideSubAddEditModal";

import PageGuideHeader from "./PageGuideHeader";
import PageGuideSideBar from "./PageGuideSideBar";
import PageGuideFiles from "../PageGuide/PageGuideFiles";

import copyToClipboard from "../../../../providers/copyToClipboard";

const PageGuideContent = ({ history, match, permission }) => {
    ...
    const {
        ...
        res => {
            console.log("guide_new_conent_", res.data);
            // if(res.seccess){
    ...
    const showModalEditGuide = data => {
        console.log("showModalEditGuide", data);
        setAddOnly(false);
        setUpdateData(data);
        setShowAddEditContentModal(!showAddeEditContentModal);
    };
    ...
    return (
        <div className="app">
            ...
            <div>
                <PageGuideHeader history={history} match={match} />
            </div>
                                ...
                                {cardSubGuides.length != 0 &&
                                    cardSubGuides.map((subGuide, key) => {
                                        return (
                                            ...
                                            >
                                                <Link
                                                    to={`/admin-guides/${match.params.id}/sub/${subGuide.id}`}
                                                    style={{
                                                        fontSize: "20px",
                                                        color: "#FE6933",
                                                        marginLeft: "30px"
