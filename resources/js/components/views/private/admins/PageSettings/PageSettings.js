...
const PageSettings = ({ history }) => {
    ...
    const {
        ...
        "setting_user_data",
        res => {
            console.log('setting_user_data', res)
            setLoginData(res.data);
            let type = res.data.image_url ? res.data.image_url.split('/') : '';
            // setImg(window.location.origin + "/storage/" + .data.image_url);
            setImg(
                type[0] == 'https:' ?
                    res.data.image_url
                    : res.data.image_url ?
                        window.location.origin +"/storage/"+ res.data.image_url :
