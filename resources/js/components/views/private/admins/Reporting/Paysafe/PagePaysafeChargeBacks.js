...
const PagePaysafeMonthlyChargeBacks = ({ history, match }) => {
    ...
    const [dataTableInfo, setDataTableInfo] = useState({
        ...
        page_number: 1,
        page_size: "20",
        column: localStorage.account_paysafe_chargeBacks_table_sort_column,
        order: localStorage.account_paysafe_chargeBacks_table_sort_order,
    ...
    const {
        mutate: mutateChargeBacks,
        isLoading: isLoadingChargeBacks
    } = useAxiosQuery(
        "POST",
        "api/v1/paysafe/chargebacks/filtered",
        "chargeBacks_merchant_list"
    );
    ...
    useEffect(() => {
        ...
        localStorage.account_paysafe_chargeBacks_table_sort_order =
            dataTableInfo.order;
        getChargeBacksList();
        return () => {};
    }, [dataTableInfo]);
    ...
    let ExportFileName =
        "Reporting Paysafe Chargebacks- " + moment().format("YYYY-MM-DD");

    const getChargeBacksList = () => {
        mutateChargeBacks(
    ...
    const [showTableColumnSettings, setShowTableColumnSettings] = useState({
        show: false,
        data: localStorage.column_settings_account_paysafe_chargeBacks_table
            ? JSON.parse(
                  localStorage.column_settings_account_paysafe_chargeBacks_table
              )
            : [
                  {
    ...
    const checkIfDefault = column => {
        if (localStorage.account_paysafe_chargeBacks_table_sort_column) {
            if (
                localStorage.account_paysafe_chargeBacks_table_sort_column ==
                column
            ) {
    ...
    const {
        ...
        "POST",
        "api/v1/paysafe/chargebacks/bulkinsert",
        "paysafe_get_merchant_list"
    );

    const handleGetChargeBacksList = e => {
        mutateGetDepositList(
            { merchant_number: match.params.merchant_number },
            {
                onSuccess: res => {
                    notification.success({
                        message: "ChargeBacks Data Updated"
                    });
                    getChargeBacksList();
                },
                onError: err => {
    ...
    useEffect(() => {
        let col_sizes = localStorage.chargebacks_table_col_sizes
            ? JSON.parse(localStorage.chargebacks_table_col_sizes)
            : null;
    ...
    useEffect(() => {
        if (!localStorage.chargebacks_table_col_sizes) {
            var a = {
                "Merchant Number": 168,
                Amount: 105,
                "Family Id": 114,
                ...
                <Row>
                    <Col md={24}>
                        <Card
                            ...
                            extra={
                                <Button
                                    ...
                                    onClick={e => handleGetChargeBacksList()}
                                >
                                    Refresh Chargebacks List
                                </Button>
                            }
                        >
                            <Row>
                                        ...
                                        <Table.Column
                                            ...
                                            title={
                                                <CustomTableTitle
                                                    ...
                                                    localStorageKey="chargebacks_table_col_sizes"
                        ...
                        <TableColumnSettings
                            ...
                            localStorageKey="column_settings_account_paysafe_chargeBacks_table"
                        />
                    </Col>
                </Row>
            </div>
...
export default PagePaysafeMonthlyChargeBacks;
